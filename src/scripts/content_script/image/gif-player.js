...
export default function() {
  if (store.getters.gif_player === 1 && store.getters.block_images === 0) {
    ...
    if (exclude.indexOf(store.state.hostname) === -1) {
      const gifEls = document.querySelectorAll('img[src*=".gif"]');
      const iframeEls = document.querySelectorAll('iframe[src*="giphy.com/embed/"]');
      if (gifEls.length > 0 || iframeEls.length > 0) {
