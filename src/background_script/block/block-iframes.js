...
import sanitizeEmbedUrl, {
  isUrlSanitized,
} from '../../utils/sanitize-embed-video-url'
...
export function blockIframes() {
  const action = (details) => {
    ...
    if (
      store.getters.getOption('video_clicktoload', tabId) === 1 &&
      !isUrlSanitized(url)
    ) {
      ...
      const sanitizedUrl = sanitizeEmbedUrl(
        url,
        false,
        true,
        store.getters.getOption('video_quality', tabId),
      )
